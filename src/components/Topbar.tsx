...
export function Topbar({ activeTab, onChange, onConfig, onOpenPalette, config }: Props) {
  ...
  return (
    <header
      className="sticky top-0 z-30 h-14 flex items-center gap-3 px-4 border-b border-border bg-[var(--color-canvas)]"
    >
      {/* Brand */}
      <div className="flex items-center gap-2.5 shrink-0">
        <div className="w-7 h-7 rounded-md bg-accent flex items-center justify-center" aria-hidden="true">
          <Calculator size={16} className="text-[var(--color-canvas)]" />
        </div>
      ...
      {/* Pills (desktop) */}
      <nav
        className="hidden md:flex flex-1 items-center gap-1 overflow-x-auto scrollbar-thin px-2"
        role="tablist"
        aria-label="Mercados"
      >
        {TABS.map((t, i) => {
      ...
      <div className="ml-auto flex items-center gap-2 shrink-0">
        <button
          type="button"
          onClick={onConfig}
          className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-border bg-surface text-[11px] font-mono font-semibold text-text-muted hover:border-hairline-strong transition-colors"
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 h-9 px-2.5 rounded-md border border-border text-text-muted hover:border-hairline-strong hover:text-text-primary transition-colors"
