...
const EXAMPLE_MAP: Record<string, Partial<Record<string, string>>> = {
  "nres-1x2": {
    "nres-type": "1X2 / Moneyline",
    "nres-name": "1X2 / Moneyline",
    "nres-odds": "2.50,3.30,2.80",
    "nres-sel": "0",
    "nres-your": "2,65",
  },
  "nres-ou": {
    "nres-type": "Over/Under",
    "nres-name": "Over/Under",
    "nres-odds": "1.95,1.95",
    "nres-sel": "0",
    "nres-your": "2,05",
  },
  ...
  "combo-boost": {
    ...
    "combo-corr": "false",
    "combo-legs":
      "3|0|1.598,4.470,5.310;3|2|5.260,3.650,1.746;3|0|1.097,10.780,20.200;3|0|2.100,3.420,3.760",
  },
  "poi-builder": {
    "poi-h": "1,80",
    "poi-d": "3,60",
  ...
  "poi-playerprop": {
    ...
    "poi-rho": "-0,13",
    "poi-legs":
      "homewin|||||||||||||||;playerprop||||||||1,31|2,41|6,60|14,00|40,00|home|0,54|",
  },
  "poi-tennis": {
    "poi-sport": "tennis",
    "poi-mlA": "1,33",
  ...
  "poi-tennis-prop": {
    ...
    "poi-bestOf": "3",
    "poi-legs":
      "matchWinner||A|||||||||||||||;totalGamesOver|20,5|||||||||||||||;firstSetWinner||A|||||||||||||||",
  },
  "asia-total": {
    "asia-mode": "total",
    "asia-cal-line": "2,5",
...
const TAB_LABELS: Record<TabId, { title: string; sub: string }> = {
  nres: {
    title: "N Resultados",
    sub: "Mercados de 2+ resultados (1X2, Over/Under, etc.)",
  },
  ...
  poi: {
    title: "Bet Builder",
    sub: "Combine seleções com correlação implícita",
  },
...
function App() {
  ...
  const { result, isLoading } = useCalculator(
    inputs,
    config,
    activeTab,
    isLazyTab ? calcTrigger : undefined,
  );
  ...
  const loadExample = useCallback((key: string) => {
    ...
    setInputs((prev) => {
      ...
      Object.entries(example).forEach(([k, v]) => {
        if (v !== undefined)
          next[k] = RAW_LIST_FIELDS.has(k) ? v : v.replace(/,/g, ".");
      });
      return next;
    });
  }, []);
  ...
  const resetTab = useCallback(() => {
    ...
    setInputs((prev) => {
      const hadContent = Object.keys(prev).some(
        (k) =>
          pfx.some((p) => k.startsWith(p)) &&
          !(k in DEFAULT_INPUTS) &&
          prev[k] !== "",
      ...
      for (const k of Object.keys(next))
        if (pfx.some((p) => k.startsWith(p))) next[k] = "";
      return next;
    });
  }, [activeTab]);
  ...
  const tabContent = useMemo(() => {
    ...
    const common = {
      ...
      isLoading,
      hideCalcButton: hideCalc,
    };
    switch (activeTab) {
      case "nres":
        return <NResultsTab {...common} result={result} config={config} />;
      case "props":
        return <PropsTab {...common} />;
      case "proxy":
        return <ProxyTab {...common} />;
      case "aub":
        return <AubTab {...common} />;
      case "combo":
        return <ComboTab {...common} />;
      case "poi":
        return <BetBuilderTab {...common} />;
      case "asia":
        return <AsianTab {...common} />;
    }
  }, [
    activeTab,
    inputs,
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 xl:p-8 pb-20 md:pb-6 max-w-[1600px] mx-auto w-full">
          <div className="mb-5">
            <h1 className="t-headline text-text-primary">
              {TAB_LABELS[activeTab].title}
            </h1>
            <p className="t-body-sm text-text-muted mt-1">
              {TAB_LABELS[activeTab].sub}
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_560px]">
            <div className="min-w-0 animate-fade-in space-y-4">
              ...
              {result && !("err" in result) && (
                <VizSection result={result} showFairProbabilities={activeTab !== "nres"} />
              )}
            </div>
            <aside className="hidden lg:block">
              <div className="sticky top-[calc(3.5rem+24px)] pr-1 space-y-4">
                ...
                {(!result || (result && "err" in result)) && (
                  <div className="panel">
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                      <BankRow
                        label="Banca"
                        value={`R$ ${config.bank.toLocaleString("pt-BR")}`}
                      />
                      <BankRow
                        label="Unidade"
                        value={`R$ ${config.unit.toLocaleString("pt-BR")}`}
                      />
                      <BankRow label="Fração Kelly" value={`${config.frac}×`} />
                      <BankRow
                        label="Teto"
                        value={`${(config.cap * 100).toFixed(1)}%`}
                      />
                      <BankRow
                        label="Piso"
                        value={`${(config.floor * 100).toFixed(2)}%`}
                      />
                      <BankRow
                        label="Edge mín."
                        value={`${(config.edgemin * 100).toFixed(1)}%`}
                      />
                    </dl>
                  </div>
                )}
                ...
                {result &&
                  "err" in result &&
                  tabHasContent(inputs, activeTab) && (
                    <div
                      className="panel border-danger/30"
...
function BankRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      <dt className="text-[10px] text-text-muted uppercase tracking-wider font-semibold">
        {label}
      </dt>
      <dd className="font-mono text-xs font-medium text-text-primary mt-0.5">
        {value}
      </dd>
    </div>
  );
